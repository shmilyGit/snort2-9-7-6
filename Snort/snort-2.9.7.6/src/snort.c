...
static void InitGlobals(void)
{
    memset(&pc, 0, sizeof(pc));		/** SGL:PacketCount pc; 包统计 **/

    InitNetmasks();		/** SGL0003:初始化掩码 **/
    InitProtoNames();	/** SGL0004:初始化协议名称 **/
#ifdef SIDE_CHANNEL
    pthread_mutex_init(&snort_process_lock, NULL); /** SGL:? **/
#endif
}
...
SnortConfig * SnortConfNew(void)
{
    ...
    sc->pcre_match_limit_recursion = 1500; /** SGL:recursion 递归 **/
...
static void InitNetmasks(void)
{
    ...
    netmasks[0]  = 0x00000000; // 00000000, ==> 0.0.0.0 
    netmasks[1]  = 0x80000000; // 10000000, ==> 128.0.0.0 
    netmasks[2]  = 0xC0000000; // 11000000, ==> 192.0.0.0 
    netmasks[3]  = 0xE0000000; // 11100000, 
    netmasks[4]  = 0xF0000000; // 11110000, 
    netmasks[5]  = 0xF8000000; // 11111000, 
    netmasks[6]  = 0xFC000000; // 11111100, 
    netmasks[7]  = 0xFE000000; // 11111110, 
    netmasks[8]  = 0xFF000000; // 11111111, ==> 255.0.0.0
    ...
    netmasks[31] = 0xFFFFFFFE; //mask ==> 255.255.255.254
    netmasks[32] = 0xFFFFFFFF; //mask ==> 255.255.255.255
}
...
static void InitProtoNames(void)
{
    ...
    if (protocol_names == NULL)
        protocol_names = (char **)SnortAlloc(sizeof(char *) * NUM_IP_PROTOS);  /** SGL: NUM_IP_PROTOS = 256 **/
            ...
            default:
            {
                ...
                if (pt != NULL)
                {
                    ...
                    for (j = 0; j < strlen(protocol_names[i]); j++)
                        protocol_names[i][j] = toupper(protocol_names[i][j]); /** SGL:协议名称转为大写 **/
                }
                else
                {
                    char protoname[10];
...
{
    InitSignals();	/** SGL0001:信号处理初始化 **/

#if defined(NOCOREFILE) && !defined(WIN32)
    SetNoCores();	/** SGL0002:进程资源设置 **/
#else
...
# endif /* LINUX, BSD, SOLARIS */
    ...
    SnortAddSignal(SIGTERM, SigExitHandler, 1); // SIGTERM 是没有参数的KILL 信号
